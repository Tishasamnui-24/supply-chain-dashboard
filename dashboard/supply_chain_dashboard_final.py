...
def load_master():
    with closing(sqlite3.connect(DB_FILE)) as con:
        ...
        order_level = pd.read_sql_query(
            """
            SELECT order_id, customer_id, warehouse_id, order_date, order_status,
                   net_sales, revenue_completeness_status, delivery_anomaly_flag
            FROM order_features
            """, con)
    ...
    # Order-level table — includes EVERY order, even the 1,530 that have zero
    # line items ("No Order Details"). fact_order_items alone would silently
    # drop these, undercounting them on the Data Quality page.
    order_level["order_date"] = pd.to_datetime(order_level["order_date"], dayfirst=True, errors="coerce")
    order_level = order_level.merge(dim_c, on="customer_id", how="left")
    order_level = order_level.merge(dim_w, on="warehouse_id", how="left")

    return items, inv, ship, order_level
...
try:
    items, inv, ship, order_level = load_master()
...
def filter_orders(order_df, cat, sup, wh, seg, yr, valid_order_ids):
    """Filter the order-level table (which includes zero-item 'No Order
    Details' orders). Warehouse/Segment/Year are order-level attributes and
    are applied directly. Category/Supplier are item-level attributes, so
    when either is set, orders are further restricted to those that have at
    least one matching line item — meaning a 'No Order Details' order (which
    has no line items at all) correctly drops out of that view, since it
    can't be attributed to a specific category or supplier."""
    out = order_df
    if wh != "All": out = out[out["warehouse_name"].eq(wh)]
    if seg != "All": out = out[out["segment"].eq(seg)]
    if yr != "All": out = out[out["order_date"].dt.year.eq(int(yr))]
    if cat != "All" or sup != "All":
...
order_meta_f = filter_orders(order_level, f_category, f_supplier, f_warehouse, f_segment, f_year, _filtered_order_ids)
...
# ------------------------------------------------------------
    ...
    ord_level = order_meta_f.dropna(subset=["order_date"])
